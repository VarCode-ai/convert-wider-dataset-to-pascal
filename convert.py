...
def parseImFilename(imFilename, imPath, type):

    folder, basename = imFilename.split('/')
    im = Image.open(os.path.join(imPath, basename))
    if not os.path.exists("ImageSets/Main/"):
         os.makedirs("ImageSets/Main/")
    with open('ImageSets/Main/{}.txt'.format(type), 'a') as f:
        f.write('{}\n'.format(basename.replace('.jpg','')))
    width, height = im.size
    return folder, basename, imFilename, str(width), str(height)

def convertWFAnnotations(annotationsPath, targetPath, imPath, type):
    ann = None
    basename = ''
    with open(annotationsPath) as f:
        while True:
            ...
            if imFilename:
                folder, basename, path, width, height = parseImFilename(imFilename, imPath,type)
                ann = createAnnotationPascalVocTree(folder, basename, os.path.join(imPath, basename), width, height)
...
if __name__ == '__main__':
    ...
    PARSER.add_argument('-ap', '--annotations-path', help='the annotations file path. ie:"wider_face_train_bbx_gt.txt".')
    PARSER.add_argument('-tp', '--target-path', help='the target directory path where XML files will be copied. ie:"Annotations"')
    PARSER.add_argument('-ip', '--images-path', help='the images directory path. ie:"./JPEGImages"')
    PARSER.add_argument('-t', '--type', help='train(default)/test/val', default='train')
    ARGS = vars(PARSER.parse_args())

    convertWFAnnotations(ARGS['annotations_path'], ARGS['target_path'], ARGS['images_path'],ARGS['type'])
